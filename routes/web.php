<?php

use Illuminate\Support\Facades\Route;
use App\Http\Controllers\Auth\TwoFactorController;
use App\Http\Controllers\HomeController;
use App\Http\Controllers\PersonaController;
use App\Http\Controllers\CooperativaController;
use App\Http\Controllers\VentaController;
use App\Http\Controllers\UserController;
use App\Http\Middleware\AssignRoleMiddleware;

/*
|--------------------------------------------------------------------------
| Rutas de Autenticación 2FA
|--------------------------------------------------------------------------
|
| - /2fa/enable: activar 2FA (muestra QR)
| - /2fa/verify: validar activación de 2FA
| - /2fa/validate: obligatorio después de login
|
*/

// Pantalla de validación 2FA después de login (NO requiere auth)
Route::get('/2fa/validate', [TwoFactorController::class, 'showValidate'])
    ->name('2fa.validate');

// Rutas protegidas para activar el 2FA
Route::middleware('auth')->group(function () {

    // Mostrar QR y activar 2FA
    Route::get('/2fa/enable', [TwoFactorController::class, 'enable2fa'])
        ->name('2fa.enable');

    // Verificar código del QR al activar 2FA
    Route::post('/2fa/verify', [TwoFactorController::class, 'verify'])
        ->name('2fa.verify');

    // Validar código 2FA después del login
    Route::post('/2fa/validate', [TwoFactorController::class, 'validate2fa'])
        ->name('2fa.validate.post');
});

/*
|--------------------------------------------------------------------------
| Rutas de Autenticación
|--------------------------------------------------------------------------
*/
Auth::routes();

/*
|--------------------------------------------------------------------------
| Home
|--------------------------------------------------------------------------
*/
Route::get('/', [HomeController::class, 'index'])->name('home');
Route::get('/home', [HomeController::class, 'index'])->name('home');

/*
|--------------------------------------------------------------------------
| Rutas para Admin y Cliente
|--------------------------------------------------------------------------
*/
Route::middleware([AssignRoleMiddleware::class . ':admin,cliente'])->group(function () {
    Route::resource('personas', PersonaController::class);
    Route::resource('ventas', VentaController::class);
    Route::get('ventas/resumen', [VentaController::class, 'resumen'])->name('ventas.resumen');
});

/*
|--------------------------------------------------------------------------
| Rutas solo Admin
|--------------------------------------------------------------------------
*/
Route::middleware([AssignRoleMiddleware::class . ':admin'])->group(function () {
    Route::resource('cooperativas', CooperativaController::class);
    Route::put('cooperativas/{cooperativa}', [CooperativaController::class, 'update']);
    Route::get('/users', [UserController::class, 'index'])->name('users.index');
    Route::put('/users/{id}/role', [UserController::class, 'updateRole'])->name('users.updateRole');
    Route::delete('/users/{id}', [UserController::class, 'destroy'])->name('users.destroy');
});
